...
CLI for lightweight MarkLogic administration, debugging, and exploration. Invoked as `marklogic-tool`.
...
|-----------|---------|--------------|
| 0 | Success | Proceed |
| 1 | General/not-found error | Report to user |
| 2 | Usage/syntax error | Fix command syntax (likely wrong arg order) |
| 3 | Auth/input error | Check profile, credentials |
| 5 | Connection refused | Host reachable but port wrong/closed — check port config |
| 6 | Timeout (unreachable host) | Host unreachable or very slow — retry once then report |

- **Retry**: Only transient errors (exit 5, 6). One retry, then report failure.
- **Do not retry**: Auth errors (3), usage errors (2), general errors (1).
